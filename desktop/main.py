...
if __name__ == "__main__":
  ...
  # Check if user provided arguments (skip the script name)
  has_args = len(sys.argv) > 1
  
  # Special commands that don't require full argument parsing
  ...
  # If no arguments provided, show the GUI menu
  if not has_args:
    def launch_from_menu(year, round_number, session_type):
      """Callback to launch replay from menu selection"""
      main(year, round_number, playback_speed=1, session_type=session_type, visible_hud=True)
    
    print("Launching GUI menu...")
    show_menu(launch_from_menu)
    sys.exit(0)
  
  # Otherwise, parse CLI arguments (original behavior)
  if "--year" in sys.argv:
