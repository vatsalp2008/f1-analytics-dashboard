...
def get_season_events(year):
    """
    Get all events for a given year with metadata for GUI menu.
    Returns a list of event dictionaries with round number, name, location, date, and sprint info.
    """
    enable_cache()
    schedule = fastf1.get_event_schedule(year)
    
    # Determine sprint format name based on year
    sprint_name = 'sprint_qualifying'
    if year == 2023:
        sprint_name = 'sprint_shootout'
    if year in [2021, 2022]:
    ...
    for _, event in schedule.iterrows():
        ...
        has_sprint = event['EventFormat'] == sprint_name
        ...
        event_dict = {
            ...
            'date': str(event['EventDate'])[:10] if 'EventDate' in event else '',
